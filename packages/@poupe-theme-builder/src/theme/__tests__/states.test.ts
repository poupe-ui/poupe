...
describe('theme state variants', () => {
  ...
  describe('makeCustomStateVariants', () => {
    it('should generate state variants for custom colors', () => {
      ...
      expect(darkVariants['on-brand-disabled']).toBeInstanceOf(Hct);
    ...
    it('should handle colors without containers', () => {
      ...
      expect(variants['simple-container-hover' as any]).toBeUndefined();
    });
    ...
    it('should handle complex custom color sets', () => {
      ...
      // Check on-disabled states
      expect(variants['on-brand-disabled']).toBeInstanceOf(Hct);
      expect(variants['on-brand-container-disabled']).toBeInstanceOf(Hct);
    });
  });
});

...
type StandardStateVariants = {
  ...
  [K in StandardInteractiveColorKey as `on-${K}-disabled`]: Hct;
};
...
export function makeStandardStateVariants(colors: StandardDynamicColors): StandardStateVariants {
  ...
  for (const [baseKey, onKey] of colorPairs) {
    ...
    if (baseColor && onColor) {
      ...
      variants[`on-${onKey.slice(3)}-disabled`] = states.onDisabled;
    }
  }
...
type CustomStateVariantKey<T extends string> = 
  | `${T}-hover` | `${T}-focus` | `${T}-pressed` | `${T}-dragged` | `${T}-disabled` | `on-${T}-disabled`
  | `${T}-container-hover` | `${T}-container-focus` | `${T}-container-pressed` | `${T}-container-dragged` | `${T}-container-disabled` | `on-${T}-container-disabled`;
...
export function makeCustomStateVariants<K extends string>(
  ...
  for (const colorName of colorNames) {
    ...
    if (customColors[baseKey] && customColors[onKey]) {
      ...
      variants[`on-${colorName}-disabled`] = states.onDisabled;
    }
    ...
    if (customColors[containerKey] && customColors[onContainerKey]) {
      ...
      variants[`on-${colorName}-container-disabled`] = states.onDisabled;
    }
  }

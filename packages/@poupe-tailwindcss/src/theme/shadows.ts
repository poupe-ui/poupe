 ...
 * @returns A tuple with two CSS rule objects: regular shadows and inner shadows
 * @remarks
 ...
 * 3. Inner shadows (--inner-shadow-*): For inset box shadows
 *
 * Regular and drop shadows include z1 (lowest) through z5 (highest) elevation levels,
 * each with carefully calibrated opacity and offset values. Inner shadows provide a
 * pressed/inset effect with a single default style.
 *
 * All shadows use the theme's shadow color variable with appropriate opacity levels
 * to maintain visual consistency while allowing for theme customization.
 */
export const makeShadows = (theme: Theme): [CSSRules, CSSRules] => {
  ...
  // Level 4: High elevation - for important dialogs and modals
  ...
  const inner = `inset 0 2px 4px 0 ${c20}`;

  // No shadow - explicitly remove shadows
  const none = '0 0 rgb(0, 0 / 0, 0)';
  ...
  // Define inner shadow variations
  const innerShadows: CSSRules = {
    '*': 'initial',
    'DEFAULT': inner,
    none,
  };

  return [shadows, innerShadows];
};
 ...
 * @returns An array of three CSS rule objects with prefixed variables:
 *   1. Box shadow variables (--shadow-*)
 *   2. Drop shadow variables (--drop-shadow-*)
 *   3. Inner shadow variables (--inner-shadow-*)
 */
export const makeShadowRules = (theme: Theme): CSSRules[] => {
  const [shadows, innerShadows] = makeShadows(theme);

  return [
    renameRules(shadows, (s: string) => defaultPrefix(s, '--shadow')),
    renameRules(shadows, (s: string) => defaultPrefix(s, '--drop-shadow')),
    renameRules(innerShadows, (s: string) => defaultPrefix(s, '--inner-shadow')),
  ];
};

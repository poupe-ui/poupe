...
export const makeShadows = (theme: Theme): [CSSRules, CSSRules] => {
  ...
  const rgbValues = `var(${shadowRGBVariable})`;

  // Helper function to create color with opacity using rgba()
  const color = (opacity: number) => `rgba(${rgbValues}, ${opacity.toFixed(2)})`;
...
export const makeShadowRules = (theme: Theme): CSSRules[] => {
  const [shadows, insetShadows] = makeShadows(theme);
